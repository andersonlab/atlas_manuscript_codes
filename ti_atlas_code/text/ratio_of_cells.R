...
all$direction_regulation <- factor(all$direction_regulation, levels=c("down-regulated", "up-regulated"))

table(is.na(all$qvalue_bh_allcelltypes))

check <- all %>% group_by(freeze, label) %>% dplyr::summarise(n = n())
