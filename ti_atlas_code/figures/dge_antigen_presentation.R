...
# 
 ...
 file1="/home/ubuntu/ubuntu/sc_ibd_project/ti_atlas/manuscript/ti_atlas_code/figures/data/Reactome/CLASS_I_MHC_MEDIATED_ANTIGEN_PROCESSING_PRESENTATION.tsv"
 oo2=read.table(file1, sep="\t", header=TRUE, comment.char = "",check.names = FALSE, stringsAsFactors = FALSE)
dff=data.frame(annot_id="CLASS_I_MHC_MEDIATED_ANTIGEN_PROCESSING_PRESENTATION",
                gene_symbol= unique(sapply(str_split(oo2$MoleculeName, " "), "[[", 2)), stringsAsFactors=FALSE)
...
library(ggplot2)
plt <- ggplot2::ggplot(df9, ggplot2::aes(x=gene_symbol, y=label, fill=log2fc)) 
plt <- plt + geom_tile() 
...
ggsave(plt, filename=paste("dge_antigen_presentation.pdf"), width=20, height =14, device = cairo_pdf, dpi=200)
