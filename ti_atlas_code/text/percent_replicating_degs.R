...
# You are taking only same sign DEGS
check=check[sign(check$Discovery)==sign(check$Replication),] 

length(unique(check[check$hit_type %in% c("Discovery", "Discovery & Replication"),]$gene_symbol))
length(unique(check[check$hit_type %in% c("Replication", "Discovery & Replication"),]$gene_symbol))

ll=list()
labels=unique(check$label)
for(i in 1:length(unique(check$label))){
  #i=1
  #print(i)
  d_genes=unique(check[(check$hit_type %in% c("Discovery", "Discovery & Replication") & check$label==labels[i]),]$gene_symbol)
  r_genes=unique(check[(check$hit_type%in% c("Replication", "Discovery & Replication") & check$label==labels[i]),]$gene_symbol)
  
  
  ll[[i]]=c(length(intersect(d_genes, r_genes)), length(d_genes))
  names(ll[[i]]) = c("replicating", "d_genes")
  #length(intersect(d_genes, r_genes))/length(d_genes)*100

}
names(ll) <- unique(check$label)
library(plyr)
df9 <- ldply(ll, data.frame)
#pp=as.data.frame(do.call(cbind, df9))
df9$type=rep(c("replicating", "d_genes"), times=49)
names(df9) <- c("label", "n_genes", "type")

# Percentage of replicating DEGs per cell type
papa <- df9 %>% group_by(label) %>% dplyr::summarise(perc=n_genes[type=="replicating"]/n_genes[type=="d_genes"]*100)

# Total percent of replicating DEGs in discovery
papa1 <- df9 %>% dplyr::summarise(sum_intersect=sum(n_genes[type=="replicating"]), sum_d_genes=sum(n_genes[type=="d_genes"]))
# 
#round(papa1$sum_intersect/papa1$sum_d_genes*100,0)


check3=data.frame(table(check2$gene_symbol))
