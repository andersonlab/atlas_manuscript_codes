...
df_sig$significant <- TRUE

df_sig=df_sig[!is.na(df_sig$annot_id),]
df_sig1=df_sig[,c("annot_id", "label", "qvalue_bh", "ES", "freeze", "significant")]
names(df_sig1) <- c("pathway", "cell_type", "qvalue_bh", "ES", "cohort", "significant")
nrow(df_sig1)


a=length(unique(df_sig[df_sig$freeze=="Discovery",]$annot_id))
b=length(unique(df_sig[df_sig$freeze=="Replication",]$annot_id))
a
b
a+b

write.table(file="/home/ubuntu/ubuntu/sc_ibd_project/ti_atlas/figures-ti_cd_singlecell/tables/TableS5_dge_gsea.tsv", df_sig1,  sep="\t", row.names = FALSE)
